...
public class GetFloodParcelsByFilterSqlCommand
{
    ...
    public GetFloodParcelsByFilterSqlCommand(bool isDOI)
	{
		if (isDOI)
        {
            _sqlCommand =
            @"  SELECT		TOP 50		P.[Id],
				...
				WHERE					P.[AgencyID] = @p_AgencyId AND
										...
										P.[PamsPin] NOT IN(SELECT DISTINCT PamsPin FROM Flood.FloodApplicationParcel WHERE StatusId IN (4, 5)) AND
